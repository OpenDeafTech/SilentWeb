...
- Les workflows GitHub (`ci.yml`, `perf.yml`, `release.yml`) exportent désormais `PNPM_VERSION=9.12.0`, passent explicitement cette valeur à `pnpm/action-setup@v4` et conservent `package_json_file: package.json` pour éviter les erreurs “Multiple versions of pnpm specified”.
